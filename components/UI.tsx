...
export const DevTag: React.FC<{ label: string; desc?: string }> = ({
  label,
  desc,
}) => (
  <div className="absolute top-4 left-4 z-50 pointer-events-none select-none opacity-40 hover:opacity-100 transition-opacity duration-300 hidden md:block">
    <div className="flex items-center gap-2 px-3 py-1.5 rounded bg-yellow-500/10 border border-yellow-500/20 backdrop-blur-md">
      ...
      <div className="text-[10px] font-mono uppercase tracking-widest text-yellow-200/80">
        ...
        {desc && (
          <span className="ml-2 border-l border-yellow-500/20 pl-2 opacity-70 normal-case tracking-normal">
            {desc}
          </span>
        )}
      </div>
...
export const GlassCard: React.FC<ClinicalCardProps> = ({
  children,
  className = "",
  hoverEffect = false,
}) => {
  return (
    <motion.div
      ...
      whileHover={
        hoverEffect
          ? { scale: 1.01, boxShadow: "0 10px 30px -10px rgba(0,0,0,0.1)" }
          : {}
      }
...
export const HeroToastCard: React.FC<HeroToastCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`absolute z-30 ${className} will-change-transform`}
    >
      {/* Clinical Toast Style - Optimized: reduced blur on mobile to save GPU cycles */}
      <div className="flex items-center gap-3 p-3 pr-5 rounded-2xl bg-white/95 backdrop-blur-none sm:backdrop-blur-md border border-brand-blue/10 shadow-[0_4px_16px_0_rgba(30,58,138,0.08)] sm:shadow-[0_8px_32px_0_rgba(30,58,138,0.1)] w-max max-w-[85vw] sm:max-w-xs transition-all hover:bg-white">
    ...
    >
      {words.map((word, index) => (
        <motion.span
          variants={child}
          key={index}
          className="mr-[0.25em] mb-1 inline-block origin-bottom"
        >
          {word}
        </motion.span>
      ))}
          ...
          <div className="pointer-events-auto">
            <div className="relative w-full">
              ...
              <GlassCard className="flex items-center p-4 border-l-4 border-l-green-600 bg-white/95 border border-border-subtle shadow-xl">
                ...
                <button
                  onClick={onClose}
                  className="ml-2 text-text-muted hover:text-brand-navy shrink-0"
                >
                  <X size={16} />
                </button>
              </GlassCard>
...
export const BackgroundHeading: React.FC<{
  text: string;
  className?: string;
}> = ({ text, className = "" }) => {
  return (
    <div
      className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-0 pointer-events-none select-none w-full max-w-[100vw] text-center overflow-hidden ${className}`}
    >
      {/* 
      ...
      */}
      <div
        aria-hidden="true"
        className="text-[clamp(4rem,15vw,18rem)] font-black text-transparent bg-clip-text bg-gradient-to-b from-brand-navy/[0.04] to-transparent leading-none tracking-tighter uppercase font-sans whitespace-nowrap"
      >
        {text}
      </div>
    </div>
