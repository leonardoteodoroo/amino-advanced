...
export const GlassCard: React.FC<GlassCardProps> = ({ children, className = '', hoverEffect = false }) => {
  return (
    <motion.div
      className={`relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md shadow-xl w-full ${className}`}
...
export const HeroToastCard: React.FC<HeroToastCardProps> = ({ 
  ...
  return (
    ...
    >
      {/* Nubank-inspired Glass Toast */}
      <div className="flex items-center gap-3 p-3 pr-5 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] w-max max-w-[85vw] sm:max-w-xs transition-all hover:bg-white/15">
        <div className="flex items-center justify-center w-8 h-8 md:w-10 md:h-10 rounded-xl bg-[#820ad1]/30 text-[#d8aeff] shrink-0 backdrop-blur-sm border border-white/10">
          {icon}
        </div>
        <div>
          <h4 className="text-white font-bold text-xs md:text-sm leading-none mb-1">{title}</h4>
          <p className="text-gray-200 text-[10px] md:text-xs font-medium opacity-90 leading-tight">{subtitle}</p>
        </div>
      </div>
    </motion.div>
  );
        ...
        >
            <div className="pointer-events-auto">
              <div className="relative w-full">
                <div className="absolute inset-0 bg-gradient-to-r from-purple-500/20 to-blue-500/20 rounded-xl blur-lg" />
                <GlassCard className="flex items-center p-4 border-l-4 border-l-green-400">
                   ...
                   <div className="flex-1 min-w-0">
                     ...
                     <p className="text-xs text-gray-400 truncate">{subMessage}</p>
                   </div>
                   <button onClick={onClose} className="ml-2 text-gray-400 hover:text-white shrink-0">
                     <X size={16} />
                   </button>
                </GlassCard>
              </div>
  ...
  return (
    <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-0 pointer-events-none select-none w-full max-w-[100vw] text-center overflow-hidden ${className}`}>
      {/* 
         ...
         18rem = maximum size
      */}
      <h1 className="text-[clamp(4rem,15vw,18rem)] font-black text-transparent bg-clip-text bg-gradient-to-b from-white/[0.03] to-transparent leading-none tracking-tighter uppercase font-sans whitespace-nowrap">
        {text}
      </h1>
    </div>
  );
