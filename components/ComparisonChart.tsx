...
export const ComparisonChart: React.FC = () => {
  ...
  const data = [
    ...
    // but clearly labeled as 1%.
    {
      ...
      displayValue: "1",
      color: "bg-red-300",
      info: "BCAAs convert only 1% into actual protein — 99% is wasted.",
    },
    {
      label: "Whey/Soy",
      subLabel: "& Nuts",
      ...
      displayValue: "17",
      color: "bg-slate-300",
      info: "83% Nitrogen Waste (Waste products like Ammonia strain the kidneys).",
    },
    {
      label: "Meat/Fish",
      subLabel: "& Poultry",
      ...
      displayValue: "32",
      color: "bg-slate-400",
      info: "68% Waste. Requires complex digestion before absorption.",
    },
    {
      label: "Whole Eggs",
      subLabel: "",
      ...
      displayValue: "48",
      color: "bg-slate-500",
      info: "Best whole food source, but still releases 52% as waste.",
    },
    {
      label: "Advanced Amino",
      subLabel: "Formula",
      value: 99,
      displayValue: "99",
      color: "bg-[#f97316]",
      info: "99% Perfect Utilization. Virtually ZERO metabolic waste.",
    },
  ];
      ...
      <div className="w-full h-[320px] md:h-[360px] flex items-end justify-between gap-2 md:gap-6 relative px-2 md:px-6 pb-2 pt-12">
        ...
        <div className="absolute inset-x-0 top-12 bottom-8 flex flex-col justify-between pointer-events-none opacity-[0.1] z-0 px-4">
          {[100, 75, 50, 25, 0].map((tick) => (
            <div
              key={tick}
              className="w-full h-px bg-brand-navy border-t border-dashed border-brand-navy/50 relative"
            >
              <span className="absolute -left-6 -top-2 text-[9px] text-text-muted font-mono">
                {tick}%
              </span>
            </div>
          ))}
        ...
        {data.map((item, index) => (
          <div
            key={index}
            className="flex flex-col items-center justify-end h-full w-full z-10 group relative min-w-0"
          >
            {/* Value Label */}
            ...
            >
              ...
              <div
                className={`absolute -top-2 mb-3 w-40 opacity-0 group-hover:opacity-100 transition-all duration-300 z-[100] pointer-events-none 
                    ${
                      index === 0
                        ? "left-0 translate-x-0 origin-bottom-left"
                        : index === 4
                          ? "right-0 translate-x-0 origin-bottom-right"
              ...
              >
                <div className="bg-brand-navy p-2 rounded-lg text-[10px] text-white leading-tight shadow-xl border border-white/10 text-center relative">
                  ...
                  <div
                    className={`absolute bottom-0 w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-t-[6px] border-t-brand-navy
                        ${
                          index === 0
                            ? "left-4"
                            : index === 4
            ...
            <div className="mt-3 min-h-[40px] flex flex-col items-center justify-start text-center">
              <span
                className={`text-[10px] md:text-xs font-bold leading-tight block ${index === 4 ? "text-brand-navy" : index === 0 ? "text-red-500" : "text-text-secondary"}`}
              >
                {item.label}
              </span>
              {item.subLabel && (
      ...
      <div className="mt-8 md:mt-12 p-3 bg-blue-50/50 rounded-lg border border-blue-100 backdrop-blur-sm text-center mx-auto max-w-sm">
        <p className="text-[10px] md:text-xs text-text-secondary leading-relaxed">
          <span className="text-orange-600 font-bold block mb-1">
            NNU: The Clinical Gold Standard
          </span>
